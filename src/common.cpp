...
Quaterniond quaternionHamiltonProduct(Quaterniond quatLeft, Quaterniond quatRight)
{

  
  Vector3d imaginaryPartOfLeftQuaternion{quatLeft.x(),quatLeft.y(),quatLeft.z()};
  Vector3d imaginaryPartOfRightQuaternion{quatRight.x(),quatRight.y(),quatRight.z()};

  Vector3d imaginary{quatRight.w() * imaginaryPartOfLeftQuaternion + quatLeft.w() * imaginaryPartOfRightQuaternion +
      crossProductMatrix(imaginaryPartOfLeftQuaternion) * imaginaryPartOfRightQuaternion};
  Quaterniond quatProduct{quatLeft.w() * quatRight.w() -
                   imaginaryPartOfLeftQuaternion.transpose() * imaginaryPartOfRightQuaternion,imaginary(0),imaginary(1),imaginary(2)};
...
Quaterniond fromRPYToQuaternion(const EulerAngles& angles)  // yaw (Z), pitch (Y), roll (X)
{
  ...
  Quaterniond q{cy * cp * cr + sy * sp * sr,cy * cp * sr - sy * sp * cr,sy * cp * sr + cy * sp * cr,sy * cp * cr - cy * sp * sr}; 
  //q.w = cy * cp * cr + sy * sp * sr;
  //q.x = cy * cp * sr - sy * sp * cr;
  //q.y = sy * cp * sr + cy * sp * cr;
  //q.z = sy * cp * cr - cy * sp * sr;

  Quaterniond q_normilized{q.normalized()};

  return q_normilized;
}

Matrix3d quat2RotMat_fast(const Quaterniond& quat)
{
  Matrix3d RotationMatrix = Matrix3d::Zero();
  Matrix3d S = Matrix3d::Zero();
  Vector3d imaginaryPart{quat.x(),quat.y(),quat.z()};
  double realPart{ quat.w() };

  S = crossProductMatrix(imaginaryPart);

  RotationMatrix = ((realPart*realPart - imaginaryPart.transpose()*imaginaryPart)*MatrixXd::Identity(3, 3)) + (2*realPart*S) + (2*(imaginaryPart*imaginaryPart.transpose()));
